...
#ifdef CAN_EXTRA_INPUTS_COUNT   // Enables multiple input references, via CAN
                                // Currently Implemented: BASE_SPEED ("newinput"),
                                //                        INDUCED_SINE_AMPLITUDE ("can_induced_sine_amplitude")
                                //                        INDUCED_SINE_PHASE_OFFSET ("can_induced_sine_phase_offset"), Phase_offset ("newinput_phase_offset")) for brushless motor control"
// #define FIXED_SINE_BASE_SPEED 1500      // bypasses input signal and runs at a fixed (AVG_RPM + AMP_RPM * sin(AVG_RPM * dt + PHASE_OFFSET_DEG))
// #define FIXED_SINE_AMP 400              // amplitude of sine modulation
// #define FIXED_SINE_PHASE_OFFSET_DEG 0   // phase offset in degrees
#endif
...
uint32_t MAXIMUM_RPM_SPEED_CONTROL = 20000;
...
volatile uint16_t can_induced_sine_amplitude = 0;    // no synosoid at startup
volatile uint16_t can_induced_phase_offset   = 0;    // 0 degrees
uint16_t can_induced_sine_correction = 0;   

volatile char m_step = 0;
int16_t m_angle_inc;  // Mechanical Angle increment between commutation steps (deg), in multiples of 0.1 deg  
int16_t m_angle_base = 0;
int16_t m_angle_error = 0;
int16_t m_angle_index = 0; //


char use_angle_control_loop = 0; // Indicates if external angle control loop is enabled (1) or disabled (0)
int16_t minimum_angle_control_threshold = 300; // ~25% throttle
...
int32_t base_m_angle_timing = 0;
int32_t m_angle_timing = 0;
int16_t target_m_angle = 0;
...
fastPID anglePid = { // commutation speed loop time
    .Kp = 1,
    .Ki = 0,
    .Kd = 10,
    .integral_limit = 1800,
...
0, 2, 3, 5, 7, 9, 10, 12, 14, 16, 17, 19, 21, 23, 24, 26, 28, 30, 31, 33,
35, 37, 38, 40, 42, 44, 45, 47, 49, 51, 52, 54, 56, 58, 59, 61, 63, 65, 66, 68,
70, 71, 73, 75, 77, 78, 80, 82, 84, 85, 87, 89, 91, 92, 94, 96, 98, 99, 101, 103,
105, 106, 108, 110, 111, 113, 115, 117, 118, 120, 122, 124, 125, 127, 129, 131, 132, 134, 136, 137,
139, 141, 143, 144, 146, 148, 150, 151, 153, 155, 156, 158, 160, 162, 163, 165, 167, 168, 170, 172,
174, 175, 177, 179, 181, 182, 184, 186, 187, 189, 191, 193, 194, 196, 198, 199, 201, 203, 204, 206,
208, 210, 211, 213, 215, 216, 218, 220, 222, 223, 225, 227, 228, 230, 232, 233, 235, 237, 239, 240,
242, 244, 245, 247, 249, 250, 252, 254, 255, 257, 259, 261, 262, 264, 266, 267, 269, 271, 272, 274,
276, 277, 279, 281, 282, 284, 286, 287, 289, 291, 292, 294, 296, 297, 299, 301, 302, 304, 306, 307,
309, 311, 312, 314, 316, 317, 319, 321, 322, 324, 326, 327, 329, 331, 332, 334, 335, 337, 339, 340,
342, 344, 345, 347, 349, 350, 352, 353, 355, 357, 358, 360, 362, 363, 365, 367, 368, 370, 371, 373,
375, 376, 378, 379, 381, 383, 384, 386, 388, 389, 391, 392, 394, 396, 397, 399, 400, 402, 404, 405,
407, 408, 410, 412, 413, 415, 416, 418, 419, 421, 423, 424, 426, 427, 429, 431, 432, 434, 435, 437,
438, 440, 442, 443, 445, 446, 448, 449, 451, 452, 454, 456, 457, 459, 460, 462, 463, 465, 466, 468,
469, 471, 473, 474, 476, 477, 479, 480, 482, 483, 485, 486, 488, 489, 491, 492, 494, 495, 497, 498,
559, 558, 556, 555, 553, 552, 550, 549, 548, 546, 545, 543, 542, 540, 539, 537, 536, 534, 533, 531,
530, 528, 527, 525, 524, 522, 521, 520, 518, 517, 515, 514, 512, 511, 509, 508, 506, 505, 503, 502,
500, 498, 497, 495, 494, 492, 491, 489, 488, 486, 485, 483, 482, 480, 479, 477, 476, 474, 473, 471,
469, 468, 466, 465, 463, 462, 460, 459, 457, 456, 454, 452, 451, 449, 448, 446, 445, 443, 442, 440,
438, 437, 435, 434, 432, 431, 429, 427, 426, 424, 423, 421, 419, 418, 416, 415, 413, 412, 410, 408,
407, 405, 404, 402, 400, 399, 397, 396, 394, 392, 391, 389, 388, 386, 384, 383, 381, 379, 378, 376,
375, 373, 371, 370, 368, 367, 365, 363, 362, 360, 358, 357, 355, 353, 352, 350, 349, 347, 345, 344,
342, 340, 339, 337, 335, 334, 332, 331, 329, 327, 326, 324, 322, 321, 319, 317, 316, 314, 312, 311,
309, 307, 306, 304, 302, 301, 299, 297, 296, 294, 292, 291, 289, 287, 286, 284, 282, 281, 279, 277,
276, 274, 272, 271, 269, 267, 266, 264, 262, 261, 259, 257, 255, 254, 252, 250, 249, 247, 245, 244,
242, 240, 239, 237, 235, 233, 232, 230, 228, 227, 225, 223, 222, 220, 218, 216, 215, 213, 211, 210,
208, 206, 204, 203, 201, 199, 198, 196, 194, 193, 191, 189, 187, 186, 184, 182, 181, 179, 177, 175,
174, 172, 170, 168, 167, 165, 163, 162, 160, 158, 156, 155, 153, 151, 150, 148, 146, 144, 143, 141,
139, 137, 136, 134, 132, 131, 129, 127, 125, 124, 122, 120, 118, 117, 115, 113, 111, 110, 108, 106,
105, 103, 101, 99, 98, 96, 94, 92, 91, 89, 87, 85, 84, 82, 80, 78, 77, 75, 73, 71,
70, 68, 66, 65, 63, 61, 59, 58, 56, 54, 52, 51, 49, 47, 45, 44, 42, 40, 38, 37,
35, 33, 31, 30, 28, 26, 24, 23, 21, 19, 17, 16, 14, 12, 10, 9, 7, 5, 3, 2,
0, -2, -3, -5, -7, -9, -10, -12, -14, -16, -17, -19, -21, -23, -24, -26, -28, -30, -31, -33,
-35, -37, -38, -40, -42, -44, -45, -47, -49, -51, -52, -54, -56, -58, -59, -61, -63, -65, -66, -68,
-70, -71, -73, -75, -77, -78, -80, -82, -84, -85, -87, -89, -91, -92, -94, -96, -98, -99, -101, -103,
-105, -106, -108, -110, -111, -113, -115, -117, -118, -120, -122, -124, -125, -127, -129, -131, -132, -134, -136, -137,
...
void commutate()
{
    ...
    m_step++;
#ifndef USE_ANGLE_INPUT_INDEX
    if (m_step > 6 * eepromBuffer.motor_poles) m_step = 1;
#endif
// else, if AS5047P sensor connecect via INDEX input, m_step reseted at EXTI2_IRQHandler in stm32l4xxx_it.c
#endif
...
void PeriodElapsedCallback()
{
    ...
    // If motor allignet
    if (step == 1 && m_step == 1){
        m_angle_base = 0; // Software Angle Reference!
    } else {
        if (forward == 1)
        {
    ...
    // Wrap m_angle_base to [0, 359.9] interval in multiples of 0.1 deg
...
{
#ifdef CAN_EXTRA_INPUTS_COUNT
    #ifdef FIXED_SINE_BASE_SPEED
        adjusted_input              = FIXED_SINE_BASE_SPEED
        adjusted_input_amplitude    = FIXED_SINE_AMP;
        adjusted_input_phase_offset = FIXED_SINE_PHASE_OFFSET_DEG;
    #else
    // Angle Control Mode -> Three Inputs Received: Average Throttle, Sine Amplitude and Sine Phase_offset, 
    adjusted_input = newinput;
    
    can_induced_sine_correction = 1000 - abs_int16((newinput - 1047)); // Sine amplitude should me limited to maximum throttle increase
    adjusted_input_amplitude = (can_induced_sine_amplitude > can_induced_sine_correction)? can_induced_sine_correction : can_induced_sine_amplitude; // scale sine amplitude by throttle input
    adjusted_input_phase_offset = can_induced_phase_offset;
    #endif
#else  
            ...
            if (use_speed_control_loop) {
                if (drive_by_rpm) {
                    ...
                    if (use_angle_control_loop) { // make use of closed mechanical angle control loop
                        ...
                        if (adjusted_input < minimum_angle_control_threshold || zero_crosses < 100) { // Only allow angle control for high throttle values, and after motor is running stable
                            m_angle_error = 0;
                        }else
                        {
                            // 1. Obtain Phase of the motor regarding imposed offset
                            m_angle_index = m_angle_base - adjusted_input_phase_offset; 
                            if (m_angle_index >= 3600) m_angle_index -= 3600;
                            if (m_angle_index  < 0) m_angle_index += 360;

                            // 2. Compute expected increment
                            base_m_angle_timing = 3600 * map(adjusted_input, 47, 2047, MINIMUM_RPM_SPEED_CONTROL, MAXIMUM_RPM_SPEED_CONTROL) / (eepromBuffer.motor_poles / 2); // angle_rev/us units
                            m_angle_timing = 3600 * map(adjusted_input_amplitude, 47, 2047, MINIMUM_RPM_SPEED_CONTROL, MAXIMUM_RPM_SPEED_CONTROL) / (eepromBuffer.motor_poles / 2); //  rev per u second
                            target_m_angle = m_angle_base + INTERVAL_TIMER_COUNT * (base_m_angle_timing + (m_angle_timing * sineLookupTable[m_angle_index])/1000) / 60000000 ;
                            
                            if (target_m_angle >= 3600) target_m_angle -= 3600;
                            if (target_m_angle  < 0) target_m_angle += 3600;

                            m_angle_error = target_m_angle - m_angle_base;
                            if (m_angle_error >= 1800) m_angle_error -= 1800;
                            if (m_angle_error < -1800) m_angle_error += 1800;
                    ...
                    if (adjusted_input < 47) { // dead band ?
                        ...
                        input = (uint16_t)(input_override / 10000); // speed control pid override !!! MAY NEED TO BE CHANGES TO MAX_ROM_SPEED_CONTROL or something like that
                        if (input > 2047) {
                            input = 2047;
                        }
                        if (input < 48) {
...
#ifdef CAN_EXTRA_INPUTS_COUNT
            if (use_angle_control_loop && running) { // Ensure motor is running smoothly
    
                if (zero_crosses < 100) {
                    anglePid.integral = 0; // Reset integral
                    angle_controller_output = 0;
                }else{

                    angle_controller_output = doPidCalculations(&anglePid, m_angle_error, 0);

                    // if (angle_controller_output > MAXIMUM_RPM_SPEED_CONTROL / 2 ) {
                    // angle_controller_output = MAXIMUM_RPM_SPEED_CONTROL / 2;
                    // }   
                    // if (angle_controller_output < - MAXIMUM_RPM_SPEED_CONTROL / 2) {
                    // angle_controller_output = - MAXIMUM_RPM_SPEED_CONTROL / 2;
                    // }
...
#if defined(FIXED_DUTY_MODE) || defined(FIXED_SPEED_MODE) || defined(FIXED_SINE_BASE_SPEED)
    MX_IWDG_Init();
    RELOAD_WATCHDOG_COUNTER();
    inputSet = 1;
    armed = 1;
...
#ifdef FIXED_SINE_BASE_SPEED
    eepromBuffer.use_sine_start = 0;
    target_e_com_time = 60000000 / FIXED_SINE_BASE_SPEED / (eepromBuffer.motor_poles / 2);
    input = 48;
#endif
